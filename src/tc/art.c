 ...
 * it uses a tstack to keep track of items that need to be processed later
 */
...
PRIVATE push(n)
   int n;
/*
 * push item index n onto the tstack
 */
{
   if (stptr == STACKSIZE-2) {
      STACKSIZE += STACKINCR;
      tstack = (int *) realloc(tstack, sizeof(int)*STACKSIZE);
      if (! tstack) yymallocerror();
   }
   tstack[stptr++] = n;
}
...
PRIVATE int pop()
/*
 * pop an item index from the tstack and return the value
 */
{
   stptr--;
   return tstack[stptr];
...
PRIVATE init_stack()
/*
 * push the initial item index on the tstack
 ...
 *
 * the tstack always contains pointers to items that still must be processed
 * to produce a left-derivation
 * the top element must be processed first
 * if the dot appears inside a rule
 *    M : alpha N * beta
 * then the items representing the parse for beta are already on the tstack
